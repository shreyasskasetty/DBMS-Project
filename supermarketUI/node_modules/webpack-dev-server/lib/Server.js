...
function Server (compiler, options = {}, _log) {
  ...
  app.post('/productList',(req,resp)=>{
    console.log(req.body)
    const sqlquery = "select pid,name,cost,stock from product";
    console.log(sqlquery)
    try {
      connection.query(sqlquery,function(error,rows,fields){
        if(!!error){
  ...
  app.post('/secProds',(req,resp)=>{
    ...
    try {
      connection.query(sqlquery,function(error,rs,fields){
        if(!!error){
          console.log('Error in the query'); 
          resp.send({staus: false, message : "Unable to fetch data"})
        } else{
          console.log(rs)
          if (rs.length)
          {
            try {
              connection.query(sqlquery1,function(error,rows,fields){
                if(!!error){
                  ...
                  resp.send({staus: false, message : "Wrong number"})
              }
              });
            } catch (error) {
              console.log('Error in the query');
              resp.send({staus: false, message : "Unable ggto fetch data"})
        ...
        if(!!error){
          ...
          if (rows[0].phoneno)
          {
            ...
            process.stdout.on('data', function(data) { 
                resp.send({status: true,loc: data}); 
            } ) 
    ...
    try {
      connection.query(sqlquery,function(error,rows,fields){
        if(!!error){
          ...
          console.log(rows)
          resp.send({staus: true, data : rows})
      }
      });
    } catch (error) {
      console.log('Error in the query');
